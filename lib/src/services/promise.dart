...
class Promise<T> {
  ...
  Future<void> load() async {
    ...
    if (response.errorPresent) {
      if (response.error!.isTimedOut && dbOrderBy != null) {
        ...
        _current = await SqlDatabase.getAll<T>(orderBy: dbOrderBy);
      }
  ...
  Future<void> _updateDatabase(List<T> fetched) async {
    await SqlDatabase.deleteAll<T>();
    await SqlDatabase.insertAll<T>(fetched);
  }
}
...
class PagedPromise<T> extends Promise<T> {
  ...
  Future<void> load() async {
    ...
    if ((currentError?.isTimedOut ?? false) && dbOrderBy != null) {
      _finished = true;
    }
  }
  ...
  Future<List<T>> next() async {
    ...
    if (res.dataPresent) {
      ...
      if (dbOrderBy != null) SqlDatabase.insertAll<T>(data); //Don't await
      return data;
    }
