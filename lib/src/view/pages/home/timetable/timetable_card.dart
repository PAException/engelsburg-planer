...
import 'extended_timetable.dart';
    ...
    return Center(
      child: Padding(
        ...
        child: Hero(
          ...
          child: StreamBuilder<TimetableEntry>(
            initialData: entry,
            stream: entryDoc?.snapshots().map((event) => event ?? entry),
            builder: (context, snapshot) {
              var entry = snapshot.data!;

              //Strange bug: entry on second iteration(?) to equal to this.entry though entryDoc and
              //             stream are null
              if (entryDoc == null) entry = this.entry;
                    ...
                    child: ClipRRect(
                      ...
                      child: Container(
                        ...
                        child: ListTile(
                          ...
                          title: entry.subject?.data != null
                              ? Text(entry.subject!.data!.parsedName(context),
                                  textScaleFactor: 1.25)
                              : null,
                          subtitle: TimetableSubtitle(
                            entry: entry,
                            showTeacher: context.read<AppConfigState>().appType == AppType.student,
                            showClassName:
                                context.read<AppConfigState>().appType == AppType.teacher,
                          ),
