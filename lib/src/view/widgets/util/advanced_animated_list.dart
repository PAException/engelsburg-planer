...
/// Signature for the builder callback used by [AdvancedAnimatedList].
typedef AnimatedListItemBuilder = Widget Function(
    BuildContext context, int index, Animation<double> animation);

/// Signature for the builder callback used by [AdvancedAnimatedListState.removeItem].
typedef AnimatedListRemovedItemBuilder = Widget Function(
    BuildContext context, Animation<double> animation);

/// Signature for the builder callback used by [AdvancedAnimatedListState.insertItem].
typedef AnimatedListInsertedItemBuilder = Widget Function(
    BuildContext context, Animation<double> animation);
...
class _ActiveItem implements Comparable<_ActiveItem> {
  _ActiveItem.incoming(
      this.controller, this.itemIndex, this.insertedItemBuilder)
  ...
  final AnimatedListInsertedItemBuilder? insertedItemBuilder;
  int itemIndex;
...
///
/// This widget's [AdvancedAnimatedListState] can be used to dynamically insert or
/// remove items. To refer to the [AdvancedAnimatedListState] either provide a
...
///
/// {@tool dartpad}
/// This sample application uses an [AdvancedAnimatedList] to create an effect when
...
class AdvancedAnimatedList extends StatefulWidget {
  /// Creates a scrolling container that animates items when they are inserted
  /// or removed.
  const AdvancedAnimatedList({
    Key? key,
  ...
  /// inserted with [AdvancedAnimatedListState.insertItem] and less the total number of
  /// items that have been removed with [AdvancedAnimatedListState.removeItem].
  ///
  /// Implementations of this callback should assume that
  /// [AdvancedAnimatedListState.removeItem] removes an item immediately.
  ...
  /// context.
  ///
  /// This method is typically used by [AdvancedAnimatedList] item widgets that insert
  /// or remove items in response to user input.
  ///
  /// If no [AdvancedAnimatedList] surrounds the context given, then this function will
  ...
  static AdvancedAnimatedListState of(BuildContext context) {
    final AdvancedAnimatedListState? result =
        context.findAncestorStateOfType<AdvancedAnimatedListState>();
    assert(() {
      if (result == null) {
  ...
  /// See also:
  ///
  ///  * [of], a similar function that will throw if no [AdvancedAnimatedList] ancestor
  ///    is found.
  static AdvancedAnimatedListState? maybeOf(BuildContext context) {
    return context.findAncestorStateOfType<AdvancedAnimatedListState>();
  }

  @override
  AdvancedAnimatedListState createState() => AdvancedAnimatedListState();
}
...
/// animation is passed to [AdvancedAnimatedList.itemBuilder] whenever the item's widget
/// is needed.
///
...
/// can refer to the [AdvancedAnimatedList]'s state with a global key:
///
/// ```dart
...
/// ```
///
/// [AdvancedAnimatedList] item input handlers can also refer to their [AdvancedAnimatedListState]
/// with the static [AdvancedAnimatedList.of] method.
class AdvancedAnimatedListState extends State<AdvancedAnimatedList>
    with TickerProviderStateMixin<AdvancedAnimatedList> {
  final GlobalKey<SliverAnimatedListState> _sliverAnimatedListKey = GlobalKey();

  /// Insert an item at [index] and start an animation that will be passed
  /// to [AdvancedAnimatedList.itemBuilder] when the item is visible.
  ///
  ...
  void insertItem(
    ...
    AnimatedListInsertedItemBuilder? builder,
  }) {
    _sliverAnimatedListKey.currentState!
        .insertItem(index, duration: duration, builder: builder);
  }
  ...
  /// will no longer be passed to the [AdvancedAnimatedList.itemBuilder]. However the
...
///    removed.
///  * [AdvancedAnimatedList], a non-sliver scrolling container that animates items when
...
class SliverAnimatedListState extends State<SliverAnimatedList>
  ...
  Widget _itemBuilder(BuildContext context, int itemIndex) {
    ...
    if (incomingItem?.insertedItemBuilder != null) {
      //  <--
      return incomingItem!.insertedItemBuilder!(
        context,
        animation,
      );
    }
